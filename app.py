...
def render_list_editor(spreadsheet: gspread.Spreadsheet):
    ...
    # --- Definición de Columnas (Modo Plan vs Shop) ---
    ...
    if view_mode == "shop":
        # Añadir columnas de "Comprar"
        st.info("Marca los productos (✅) y añade sus precios (Precio $).")
        
        column_config["is_checked"] = st.column_config.CheckboxColumn("✅ Comprado", default=False)
        # Uso de step=0.01 y formato %.2f para forzar decimales
        ...
        # Modo Planificar
        st.info("Añade o edita los productos de tu lista. Los precios y la marca de 'Comprado' se ocultan en este modo.")
    ...
    if st.button("Guardar Cambios", type="primary"):
        with st.spinner("Guardando en Google Sheets..."):
            try:
                ...
                # --- CORRECCIÓN BUG GUARDADO EN MODO PLANIFICAR ---
                # Si las columnas 'price' o 'is_checked' no existen (solo modo plan), 
                # las añadimos para evitar KeyError.
                ...
                # AHORA APLICAMOS EL CASTING CRÍTICO A FLOAT64 ANTES DE GUARDAR
                ...
                if success:
                    ...
                    time.sleep(1)
                    st.rerun() # <-- Añadido para recargar y ver la lista ordenada
